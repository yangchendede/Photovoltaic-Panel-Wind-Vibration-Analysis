...
parfor wangle = 1:numel(ww)
    ...
    for i = 1:numel(inputparameter)
        ...
        filename = strcat(inputdir,para,"_",num2str(w),".txt");
        ...
        % 第3步：将矩阵写入新的TXT文件
        newFileName = strcat(outputdir,"matrix_",para,"_",num2str(w),".csv");
        writematrix(disp_matrix,newFileName);
    
        newFileName2 = strcat(outputdir,"matrix_","velo",outputparameter(i),"_", num2str(w),".csv");
        velo_matrix = arrayfun(@(x) sprintf('%.5f', x), velo_matrix, 'UniformOutput', false);
        writecell(velo_matrix,newFileName2);
    
        newFileName3 = strcat(outputdir,"matrix_","acce",outputparameter(i),"_", num2str(w),".csv");
